...
export default async function JournalDetailPage({
  ...
  const articles = getArticlesByJournal(slug);
  ...
  return (
    <div>
      {/* OJS-style journal masthead */}
      <section
        className="border-b border-[var(--line)] text-white"
        style={{ background: journal.coverColor }}
      >
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
          <Link href="/journals" className="text-sm text-white/70 hover:text-white">
            ← All journals
          </Link>
          <div className="mt-4 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-3 text-xs text-white/60">
                ...
                {journal.openAccess ? "Open Access" : "Subscription"}, Founded{" "}
                {journal.foundedYear}
              </p>
            </div>
            <Link
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
            ...
            <div className="space-y-6 lg:col-span-2">
              ...
              <div className="card p-6">
                ...
                <p className="mt-3 text-sm text-[var(--muted)]">
                  This journal uses <strong>{journal.reviewType}</strong> peer
                  review. Average time to first decision is about{" "}
                  {journal.avgReviewDays} days. Acceptance rate:{" "}
            ...
            <div className="mt-6 space-y-3">
              ...
              {articles.map((a) => (
                ...
                >
                  ...
                  <h3 className="mt-1 font-semibold text-[var(--ink)]">{a.title}</h3>
                  <p className="mt-1 text-sm text-[var(--muted)]">
        ...
        {tab === "archives" && (
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--muted)]">
              Browse volumes and issues (OJS-style).
            </p>
        ...
        {tab === "submit" && (
          <div className="card max-w-2xl p-6">
            ...
            <p className="mt-3 text-sm leading-relaxed text-[var(--muted)]">
              Use the ScholarOne-style wizard to enter metadata, authors,
              statements, and files. APC for this journal:{" "}
              <strong>{journal.apc}</strong>. Review model:{" "}
              <strong>{journal.reviewType}</strong>.
            </p>
